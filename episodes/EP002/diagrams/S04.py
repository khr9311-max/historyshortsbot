"""
S04 — 인과 화살표 : 두 조건의 수렴
나레이션: "조세의 은납화와 공급망 충격, 두 조건이 겹쳤습니다."
...
class S04Scene(DiagramScene):
    ...
    def build(self):
        ...
        c1 = condition("조건 1", "조세 제도", "오직 은으로만 납부 (일조편법)",
                       pict_doc(INK, height=1.15, seal=SUB))
        c2 = condition("조건 2", "외부 공급망", "17세기 글로벌 은 유입 급감",
        ...
        # --- 수렴 → 결과 ---
        result = card(
            txt("은값 폭등과 치명적 디플레이션", size=FS_LEAD, color=ACCENT),
