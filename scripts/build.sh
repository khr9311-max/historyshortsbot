#!/usr/bin/env bash
# ============================================================
#  역사 인과 쇼츠 조립 파이프라인 (색상 및 싱크 보정 버전)
#  사용법: ./scripts/build.sh EP001
#  입력:   episodes/EP001/scenes.tsv
#  출력:   render/EP001_final.mp4
# ============================================================
set -euo pipefail

EP="${1:?사용법: ./scripts/build.sh EP001}"
DIR="episodes/${EP}"
A="${DIR}/assets"
WORK="$(mktemp -d)"
...
make_still () {
  ...
  local frames=$(python -c "import math; print(int(math.ceil($FPS * $dur)))")
  local z x y
  case "$mode" in
  ...
  case "$kind" in
    ai_hero)
      if [ -f "${A}/clips/${scene}.mp4" ]; then
        ...
        make_still "${A}/images/${scene}.png" "$dst" "${move:-orbit}" "$dur"
      fi ;;
    ai_still)
    ...
    diagram)
      if [ ! -f "${A}/clips/${scene}.mp4" ]; then
        echo "Manim 렌더: ${scene}"
        python -m manim -qh --format=mp4 --resolution ${W},${H} \
          "${DIR}/diagrams/${scene}.py" "${scene}Scene" -o "${scene}" >/dev/null
...
ffmpeg -y -loglevel error \
  -i "$WORK/joined.mp4" \
  -vf "format=yuv420p,${SAT},${GRAIN},${VIGN}${SUBFILTER}" \
  -c:v libx264 -crf 18 -pix_fmt yuv420p "$WORK/video.mp4"

# --- 오디오 3레이어 + 더킹 ---------------------------------
if [ -f "${A}/vo/vo.wav" ] && [ -f "${A}/bgm/amb.wav" ] && [ -f "${A}/bgm/bgm.mp3" ]; then
  ffmpeg -y -loglevel error \
    -i "$WORK/video.mp4" \
    -i "${A}/vo/vo.wav" \
    -i "${A}/bgm/amb.wav" \
...
elif [ -f "${A}/vo/vo.wav" ]; then
  ffmpeg -y -loglevel error -i "$WORK/video.mp4" -i "${A}/vo/vo.wav" -map 0:v -map 1:a -c:v copy -c:a aac -b:a 192k -shortest "$OUT"
else
  cp "$WORK/video.mp4" "$OUT"
fi
